...
// +----------------------------------------------------------------------
// | Copyright (c) 2021-202021 All rights reserved.
// +----------------------------------------------------------------------
// | Licensed ( http://www.apache.org/licenses/LICENSE-2.0 )
// +----------------------------------------------------------------------
// | Author: woxiaoyao <1458875913@qq.com>
// +----------------------------------------------------------------------
// 1、优化代码，移除gif检测的bug,不支持gif，目前对jpg、jpeg和png支持较好
// 2、统一jpg和png保存时质量参数的意义，imagejpeg和imagepng的质量是相反的意义，imagepng质量参数参考https://www.php.net/manual/zh/function.imagepng.php
// 3、gif支持等待以后再完善
...
class Image
{
    ...
    protected function __construct(\SplFileInfo $file)
    {
         ...
         * 索引 1 包含图像高度的像素值
         * 索引 2 是图像类型的标记
         * 1 = GIF，2 = JPG，3 = PNG，4 = SWF，5 = PSD，6 = BMP，7 = TIFF(intel byte order)，8 = TIFF(motorola byte order)，9 = JPC，10 = JP2，11 = JPX，12 = JB2，* 13 = SWC，14 = IFF，15 = WBMP，16 = XBM。这些标记与 PHP 4.3.0 新加的 IMAGETYPE 常量对应。
         * 索引 3 是文本字符串，内容为“height="yyy" width="xxx"”，可直接用于 IMG 标记。
         * 对于 JPG 图像，还会多返回两个索引：channels 和 bits。
         * channels 对于 RGB 图像其值为 3，对于 CMYK 图像其值为 4。
         * bits 是每种颜色的位数。
         * 参数 mime,PHP4.3.0起支持符合该图像的 MIME 类型。此信息可以用来在 HTTP Content-type 头信息中发送正确的信息
         */
        $info = @getimagesize($file->getPathname());

        //检测图像合法性
        // if (false === $info || (IMAGETYPE_GIF === $info[2] && empty($info['bits']))) {
        if (false === $info) throw new ImageException('Illegal image file');
        if (IMAGETYPE_GIF === $info[2]) throw new ImageException('gif is not suppurt');
        ...
        if (empty($this->im)) throw new ImageException('Failed to create image resources!');
    }
    ...
    {
        if (is_string($file)) $file = new \SplFileInfo($file);
        if (!$file->isFile()) throw new ImageException('image file not exist');
     ...
     * @param bool        $interlace JPEG类型,是否对图像设置隔行扫描;png类型，是否保存完整的 alpha 通道信息
     * @return $this
     */
    public function save($pathname, $type = null, $quality = -1, $interlace = true)
    {
        //自动获取图像类型        
        ...
        // 限制质量范围(0-100)
        // jpg或jpeg是质量越高越清楚，体积越大
        // png是质量越小越清楚，体积越大
        if ($quality === -1) {
            if ('jpeg' == $type || 'jpg' == $type) $quality = 75;
            if ('png' == $type) $quality = 0;
        }
        ...
        if ('jpeg' == $type || 'jpg' == $type) {
            ...
            //设定保存完整的 alpha 通道信息
            imagesavealpha($this->im, $interlace);
